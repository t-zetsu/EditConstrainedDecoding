...
def topk_huggingface(timestep: int,
    ...
    select_ope_rewards = [[-1] * num_fill for _ in range(batch_size)]
    ...
    for sentno in range(batch_size):
        ...
        if all([x is None for x in hypotheses[rows]]):
            ...
            select_ope_rewards[sentno] = [-1] * num_fill
            continue
        ...
        select_best_ids[sentno], select_best_word_ids[sentno], select_seq_scores[sentno],\
            select_hypotheses[sentno], select_num_mets[sentno], select_ope_rewards[sentno] = _sequential_topk(timestep,
                                                                                  beam_size,
                                                                                  prune_factor,
                                                                                  sat_tolerance,
                                                                                  beta,
    ...
    return select_seq_scores, select_raw_token_idx, select_hypotheses, select_num_mets, select_ope_rewards
...
def _sequential_topk(timestep: int,
    ...
    for row, col, seq_score in zip(best_ids, best_word_ids, sequence_scores):
        ...
        sibling_ids = set(best_word_ids[best_ids==row])
        seq_score = float(seq_score)
        new_item = hypotheses[row].advance(col, sibling_ids)
    ...
    for row in range(beam_size if timestep > 0 else 1):
        ...
        for col in best_k:
            if hyp.is_valid(col):
                if col not in avoidones:
                    nextones.append(col)
        nextones = np.array(nextones)

        # Now, create new candidates for each of these items
        for col in nextones:
            if [row, col] not in hit and rank[row, col] < prune_factor:
                bp=0
                sibling_ids = set(nextones)
                new_item = hyp.advance(col, sibling_ids)
    ...
    if candidates:
        ...
        max_satisfy = max([x.hypothesis.ope_reward() for x in sorted_candidates])
        sorted_candidates = [x for x in sorted_candidates if x.hypothesis.ope_reward() >= max_satisfy - sat_tolerance]
    ...
    return (np.array([x.row for x in pruned_candidates]),
            ...
            [x.hypothesis.ope_reward() for x in pruned_candidates])