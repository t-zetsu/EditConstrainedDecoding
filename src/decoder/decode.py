...
# from utils import evaluate_sentences, use_task_specific_params, trim_batch
...
def generate_summaries_or_translations(
    ...
    **gen_kwargs,
    ...
    print(f'Decode with {model_name},{device}')
    ...
    # period_id = []
    # period_id.append(tokenizer.convert_tokens_to_ids('Ġ.')) #omli bart
    ...
    for batch, cons in tqdm(zip(list(chunks(examples, batch_size)), list(chunks(constraints_list, batch_size)))):
        constraints = init_batch(raw_constraints=cons,
                                 ...
                                 eos_id=eos_ids,
                                 ope_weight=[args.w_del, args.w_keep, args.w_repl])
        if "t5" in model_name:
        ...
        summaries, hyps = generate(self=model,
                             input_ids=input_ids,
        ...
        # print(tokenizer.batch_decode([hyp[1] for hyp in hyps], skip_special_tokens=True, clean_up_tokenization_spaces=False))
...
def run_generate():
    ...
    parser.add_argument("--score_path", type=str, required=False, help="where to save the score in json format")
    ...
    parser.add_argument('--sat_tolerance', type=float, default=2, help="minimum satisfied clause of valid candidates")
    ...
    parser.add_argument('--w_del', type=float, default=1)
    parser.add_argument('--w_keep', type=float, default=1)
    parser.add_argument('--w_repl', type=float, default=1)
    ...
    with open(args.input_file, 'r', encoding="utf-8") as f:
        examples = [json.loads(x)["src_sentence"].rstrip() for x in f.readlines()]
    with open(args.input_file, 'r', encoding="utf-8") as f:
        constraints_list = [json.loads(s)["constraint"] for s in f.readlines()]
    ...
    if args.reference_path is None:
        return
    # Compute scores
    score_fn = evaluate_sentences
    output_lns = [x.rstrip() for x in open(args.save_path).readlines()]
    reference_lns = [x.rstrip() for x in open(args.reference_path).readlines()][: len(output_lns)]
    input_lns = [x.rstrip() for x in open(args.input_path).readlines()][: len(output_lns)]
    bleu, sari, fkgl = score_fn(input_lns, reference_lns, output_lns)
    scores = {'bleu' : bleu, 'sari' : sari, 'fkgl' : fkgl}
