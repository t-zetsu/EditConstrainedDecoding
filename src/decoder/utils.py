...
def get_git_info():
    repo = git.Repo('/home/zetsu/transformers')
    repo_infos = {
        "repo_id": str(repo),
...
def evaluate_sentences(comps, simps, simp_preds, calc_simp_pred_bleu=False, calc_comp_simp_bleu=False):
    ...
    comps = [sent for sent in comps]
    if type(simps[0]) == type([]):
        simps = [[sent for sent in l] for l in simps]
    else:
        simps = [sent for sent in simps]
    simp_preds = [sent for sent in simp_preds]
    if type(simps[0]) == type([]):
        refs = simps
    else:
        refs = [simps]
    ...
    # if analysis:
    #     result = result + (corpus_analyse_operations(comps, simp_preds, refs))
    return result