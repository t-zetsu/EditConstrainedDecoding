...
class Clause:
    ...
    __slots__ = ('idx', 'positive', 'negative', 'satisfy', 'operation')
    ...
    def __str__(self):
        return f'clause(id={self.idx}, positive={self.positive}, negative={self.negative}, satisfy={self.satisfy}), operaiton={self.operation}'
...
class ConstrainedHypothesis:
    ...
    def __init__(self,
        ...
        self.ope_weight = [w/sum(ope_weight) for w in ope_weight]
        ...
        for idx, clause in enumerate(constraint_list):
            ...
            if not pos_phrases and len(neg_phrases) == 1:
                ...
                self.clauses.append(Clause(idx=idx, positive=[], negative=neg_phrases, satisfy=True, operation=0))
            # clause contains multiple negative literals or both negative and positive literals
            elif neg_phrases:
                soft_neg_pool.extend(neg_phrases)
                self.clauses.append(Clause(idx=idx, positive=pos_phrases, negative=neg_phrases, satisfy=False, operation=0))
            # clause contains only positive literals
            elif pos_phrases and not neg_phrases:
                pos_pool.extend(pos_phrases)
                self.clauses.append(Clause(idx=idx, positive=pos_phrases, negative=[], satisfy=False, operation=0))
            else:
        ...
        self.max_process = 0
        self.sat_operations = [0, 0, 0]
        self.num_operations = [0, 0, 0]
        for clause in self.clauses:
            if clause.negative:
                if clause.positive:
                    self.num_operations[2] += 1
                else:
                    self.num_operations[0] += 1
            else:
    ...
    def ope_reward(self) -> float:
        ...
        ope_score = [sat/num if num!=0 else 0 for sat, num in zip(self.sat_operations, self.num_operations)]
        reward = sum([w*score for w, score in zip(self.ope_weight, ope_score)])
    ...
    def advance(self, word_id: int, sibling_ids: Set[int]) -> 'ConstrainedHypothesis':
        """
        Updates the constraints object based on advancing on word_id.
        ...
        for clause in obj.clauses:
            if (set(sum(clause.negative+clause.positive, [])) & sibling_ids):
                if not clause.positive and clause.negative and clause.satisfy:
                    if word_id == clause.negative[0][0]:
                        sat_del = False
                    else:
                        pass

                if clause.positive and not clause.negative and not clause.satisfy:
                    if word_id == clause.positive[0][0]:
                        sat_keep = True
                    else:
                        pass

                if clause.positive and clause.negative and not clause.satisfy:
                    if word_id in sum(clause.positive, []):
                        sat_repl_pos = True
                    elif word_id == clause.negative[0][0]:
                        sat_repl_neg = False
                    else:
                        pass

        obj.sat_operations[0] += (sat_del-1)
        obj.sat_operations[1] += sat_keep
        obj.sat_operations[2] += sat_repl_pos
        obj.sat_operations[2] += (sat_repl_neg - 1)

        if obj.soft_negative_state is not None:
            for clause in obj.clauses:
                if clause.positive and clause.negative and word_id in sum(clause.positive, []):
                    clause.satisfy = True

        if obj.hard_negative_state is not None:
            temp_neg_state = obj.hard_negative_state.consume(word_id)
            for clause in obj.clauses:
                if clause.negative and word_id == clause.negative[0][0]:
                    clause.satisfy = False
...
def init_batch(raw_constraints: List[ClauseConstraintList],
               beam_size: int,
               eos_id: Union[int, list],
               ope_weight: list) -> List[Optional[ConstrainedHypothesis]]:
    """
    ...
    for i, raw_list in enumerate(raw_constraints):
        hyp = ConstrainedHypothesis(raw_list, eos_id, ope_weight)
        idx = i * beam_size
