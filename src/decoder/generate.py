...
class BeamHypotheses(object):
    ...
    def add(self, hyp, sum_logprobs, num_met, ope_reward):
        """
        ...
        if (len(self) < self.num_beams or score > self.worst_score) and len(hyp) > self.min_length:
            self.beams.append((score, hyp, num_met, ope_reward))
...
def _generate_beam_search(
    ...
    ope_rewards = [x.ope_reward() for x in constraints]

    while cur_len < max_length:
        model_inputs = self.prepare_inputs_for_generation(
            # input_ids=input_ids, past=past, attention_mask=attention_mask, use_cache=use_cache, **model_specific_kwargs
        ...
        if do_sample:
            ...
            pick_scores, pick_tokens, constraints, num_mets, ope_rewards = topk_huggingface(timestep=cur_len,
                                                                               batch_size=batch_size,
                                                                               beam_size=num_beams,
                                                                               vocab_size=vocab_size,
                                                                               pad_token_id=pad_token_id,
        ...
        for batch_idx in range(batch_size):
            ...
            if done[batch_idx]:
                ...
                next_batch_beam.extend([(0, pad_token_id, 0, None, -1, -1)] * num_beams)  # pad the batch
                continue
            ...
            for beam_token_rank, (beam_token_id, beam_token_score, constraint, num_met, ope_reward) in enumerate(
                zip(next_tokens[batch_idx], next_scores[batch_idx], constraints[batch_idx], num_mets[batch_idx], ope_rewards[batch_idx])
            ):
                ...
                if ((eos_token_id is not None) and (token_id.item() == eos_token_id)) or sentence_end:
                    ...
                    generated_hyps[batch_idx].add(
                        torch.cat((input_ids[effective_beam_id], token_id.view([1]))), beam_token_score.item(), num_met, ope_reward
                    )
                else:
                    # add next predicted token since it is not eos_token
                    next_sent_beam.append((beam_token_score, token_id, effective_beam_id, constraint, num_met, ope_reward))
            ...
            if len(next_sent_beam) < num_beams:
                if next_sent_beam:
                    ...
                    pad_candidate = (0, pad_token_id, 0, None, -1, -1)
                else:
        ...
        ope_rewards = [x[5] for x in next_batch_beam]
    ...
    for batch_idx in range(batch_size):
        ...
        for beam_id in range(num_beams):
            ...
            final_ope_reward = ope_rewards[effective_beam_id]
            generated_hyps[batch_idx].add(final_tokens, final_score, final_num_met, final_ope_reward)
    ...
    for i, hypotheses in enumerate(generated_hyps):
        ...
        # sorted_hyps = sorted(hypotheses.beams, key=lambda x: (x[2], x[0]), reverse=True) #デフォルト
        sorted_hyps = sorted(hypotheses.beams, key=lambda x: (x[3], x[0]), reverse=True)
