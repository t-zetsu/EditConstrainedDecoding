...
def read_Dataset(filename, transpose=False):
	# data = []
	...
	input = [json.loads(x) for x in open(filename, mode='r', encoding='utf-8').readlines()]

	sent1_list = []
	sent2_list = []
	for i in range(len(input)):
		if transpose:
			source = input[i]['dst_sentence']
			target = input[i]['src_sentence']
		else:
			source = input[i]['src_sentence']
			target = input[i]['dst_sentence']
	...
	# 	data.append(example(i, sent1_list[i], sent2_list[i], '0-0'))
	for i in range(len(sent1_list)):
		text_a = " ".join(sent1_list[i].split()[:256])
		text_b = " ".join(sent2_list[i].split()[:256])
		data.append(example(i, text_a, text_b, '0-0'))
	return data
...
def create_Data_Loader(data_examples, set_type='train', batchsize=16, max_seq_length=256, tokenizer=None):
...
class NeuralWordAligner(nn.Module):
	def __init__(self, args):
		...
		self.bert_model = BertModel.from_pretrained('SpanBERT/spanbert-base-cased', output_hidden_states=True, output_attentions=True)
	...
	def forward(
		...
		# print(outputs[0].size()) # bsz*512*768
		# print(all_layer_hidden_states[-1].size()) # bsz*512*768
		# print(all_layer_attention_weights[-1].size()) # bsz*12*512*512
		...
		for i in range(batch_size):
			...
			# zetsu 修正１
			# sent1_valid_ids[i] *= sent1_valid_ids[i]<256
			# sent1_valid_ids_part2 = (sent1_valid_ids[i]+min(sent2_wordpiece_length[i]+1,256))*(sent1_valid_ids[i]>0).int()
			# sent1_valid_ids_part2 *= sent1_valid_ids_part2<512
		
			# zetsu 修正２
			sent1_valid_ids[i] *= (sent1_valid_ids[i]+sent2_wordpiece_length[i]+1)<args.max_sent_length

			sent1_valid_ids_part2 = (sent1_valid_ids[i]+sent2_wordpiece_length[i]+1)*(sent1_valid_ids[i]>0).int()
			# print(sent1_valid_ids_part2 )
			# print(all_hidden_states[i].size(), all_hidden_states_b_and_a[i].size())
			...
			# zetsu 修正１
			# sent2_valid_ids_part2 *= sent2_valid_ids_part2<256
			# sent2_valid_ids[i] = (sent2_valid_ids_part2+min(sent1_wordpiece_length[i]+1,256))*(sent2_valid_ids_part2>0).int()
			# sent2_valid_ids[i] *= sent2_valid_ids[i]<512
			# zetsu 修正２
			sent2_valid_ids[i] *= (sent2_valid_ids[i])<args.max_sent_length
...
def inference(input_file, output_file):
	...
	# data = read_Word_Alignment_Dataset(input_text)
	data = read_Dataset(input_file)
	test_dataloader = create_Data_Loader(data_examples=data, set_type='test', batchsize=args.batchsize, max_seq_length=args.max_sent_length, tokenizer=tokenizer)
...
if __name__ == "__main__":
	...
	parser.add_argument("--max_sent_length", default=500, type=int)
	...
	checkpoint = torch.load(args.checkpoint, map_location=my_device)
	model.load_state_dict(checkpoint['model_state_dict'])
	model.eval()

	for split in splits:
		os.makedirs(f"{args.input_dir}/{split}", exist_ok=True)
		inference(f"{args.input_dir}/{split}.jsonl", f"{args.input_dir}/{split}/{split}.align.txt")